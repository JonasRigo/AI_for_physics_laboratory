...
1. Choose a narrow scientific task and prepare one input with a manually understood outcome.
    
2. Write the task specification and select the baseline before constructing an elaborate workflow.
    
3. Make the smallest complete version run and save its evidence record.
    
4. Evaluate it, inspect a failure, and improve the part responsible for that failure.
    
5. Ask another group to run the supplied example using only your instructions, then resolve the gaps they identify.
    
6. Upload the workflow, documentation, examples, and evidence to the shared course GitHub repository.
    
7. Prepare and deliver the 45-minute presentation, upload its materials, and reuse the results for the separate poster session.
